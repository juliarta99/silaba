    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 pt-6">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
            <div class="flex items-center gap-5">
                ...
                <div class="relative shrink-0">
                    <div class="w-20 h-20 rounded-full overflow-hidden bg-primary-100 flex items-center
                        ...
                        <span id="avatar-initials"
                              class="text-2xl font-bold text-primary-600 select-none">{{ $initials }}</span>
                        @endif
                    </div>

                    {{-- Tombol kamera --}}
                    <label for="photo-upload"
                           class="absolute bottom-0 right-0 w-7 h-7 rounded-full bg-primary-500
                                  hover:bg-primary-700 text-white flex items-center justify-center
                    ...
                    <form id="photo-form"
                          action="{{ route('district-chief.profile.photo') }}"
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mb-2">Camat {{ $districtName }}</p>
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5
                                 rounded-full
                                 {{ $districtChief->status === 'active'
                                     ? 'bg-green-100 text-success'
                                     : 'bg-red-100 text-error' }}">
                        <span class="w-1.5 h-1.5 rounded-full
                                     {{ $districtChief->status === 'active' ? 'bg-success' : 'bg-error' }}"></span>
                        {{ $districtChief->status === 'active' ? 'Aktif Menjabat' : 'Tidak Aktif' }}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5"
            ...
            <div x-show="!editing" class="space-y-4">
                ...
                <div>
                    ...
                    <div class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-10
                ...
                <div>
                    ...
                    <div class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-10 text-sm text-gray-700">
                        Kecamatan {{ $districtName }}, Kab. Badung
                    </div>
                </div>
            ...
            <div x-show="editing" x-cloak>
                <form action="{{ route('district-chief.profile.update') }}" method="POST" class="space-y-5">
                    @csrf
                    @method('PATCH')

                    <div>
                        <label class="block text-xs font-semibold text-gray-600 mb-1.5">NIP</label>
                        <input type="text" value="{{ $districtChief->nip }}" readonly
                    ...
                    <div>
                        ...
                        <input type="email" id="email" name="email"
                               value="{{ old('email', $districtChief->email) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" id="phone" name="phone"
                               value="{{ old('phone', $districtChief->phone) }}" required
                               class="w-full px-4 py-3 rounded-xl border border-gray-200 bg-gray-10 text-sm
                                      text-gray-900 focus:bg-white focus:border-primary-500 focus:ring-1
                                      focus:ring-primary-500 outline-none transition-all">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6"
            ...
            <div x-show="changingPassword" x-cloak class="mt-5 pt-5 border-t border-gray-100">
                <form action="{{ route('district-chief.profile.password') }}" method="POST"
                      class="space-y-4"
