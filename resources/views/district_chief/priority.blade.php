@extends('layouts.app')
@section('title', 'Analisis & Rekomendasi — Kecamatan ' . $districtName)
            ...
            <div class="flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-sm text-primary-200 mt-0.5">
                        ...
                        <span class="ml-1.5 text-xs bg-white/20 px-2 py-0.5 rounded-full">Kec. {{ $districtName }}</span>
                    </p>
                </div>
                {{-- Export button --}}
                <div class="shrink-0 flex items-center gap-2">
                    <a href="{{ route('district-chief.reports.priority.export') }}?format=csv"
                       class="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-white/20 hover:bg-white/30
    ...
    <div class="max-w-5xl mx-auto px-4 sm:px-6 pt-5 space-y-5">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div class="flex items-center justify-between px-5 pt-5 pb-3 border-b border-gray-50">
                ...
                <div class="flex items-center gap-3 text-xs text-gray-400">
                    ...
                    @endif
                    <a href="{{ route('district-chief.reports.priority') }}?refresh_ai=1"
            ...
            <div class="p-5 space-y-4">
                ...
                <div class="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-primary-50 border border-primary-100 mt-2">
            ...
            <div class="space-y-5">
                ...
                <div class="border border-gray-100 rounded-xl p-4">
                    <div class="flex items-start justify-between gap-3 mb-3">
                        <div class="flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold text-white shrink-0
                                        {{ $idx < 3 ? 'bg-primary-500' : 'bg-gray-300' }}">
                                #{{ $idx + 1 }}
                            </div>
                            <div>
                                <p class="text-sm font-bold text-gray-900">{{ $kat['nama'] }}</p>
                                <p class="text-xs text-gray-400">{{ $kat['pct_of_total'] }}% dari total kabupaten</p>
                            </div>
                        </div>
                        <span class="text-sm font-bold {{ $kat['completion'] >= 90 ? 'text-success' : ($kat['completion'] >= 75 ? 'text-yellow-600' : 'text-error') }}">
                    ...
                    <div class="w-full bg-gray-100 rounded-full h-1.5 mb-3">
                        <div class="h-1.5 rounded-full {{ $barClr }} transition-all" style="width:{{ $kat['completion'] }}%"></div>
                    </div>
                    ...
                    <div class="grid grid-cols-4 gap-3 text-center">
                        <div>
                            ...
                            <p class="text-sm font-bold text-success">{{ $kat['selesai'] }}</p>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <p class="text-sm font-bold text-gray-700">{{ $kat['total'] }}</p>
                        </div>
                    </div>
                </div>
                @endforeach
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div class="space-y-3">
                ...
                @php
                $rankColors = ['bg-red-500', 'bg-orange-400', 'bg-yellow-400'];
                $rankClr = $rankColors[$idx] ?? 'bg-gray-300';
                @endphp
                <div class="border {{ $dinas['pct_terlambat'] >= 15 ? 'border-red-200 bg-red-50' : 'border-gray-100 bg-white' }} rounded-xl p-4">
                    <div class="flex items-start justify-between gap-3 mb-2">
                        <div class="flex items-center gap-3">
                            <div class="w-7 h-7 rounded-full {{ $rankClr }} text-white text-xs font-bold flex items-center justify-center shrink-0">
                                #{{ $idx + 1 }}
                            </div>
                            <div>
                        ...
                        <span class="text-sm font-bold text-error">{{ $dinas['pct_terlambat'] }}%<br>
                            <span class="text-xs font-normal text-gray-400">{{ $dinas['terlambat'] }} dari {{ $dinas['total'] }}</span>
                        </span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-1.5 mb-3">
                    ...
                    <div class="grid grid-cols-3 gap-3 text-center text-xs">
                        ...
                        <div>
                            ...
                            <p class="font-bold text-gray-700">{{ $dinas['total'] }}</p>
                        </div>
                        <div>
        ...
        {{-- ══════ DISTRIBUSI PER KECAMATAN (perbandingan) ══════ --}}
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            <h2 class="text-base font-bold text-gray-900 mb-4 flex items-center gap-2">
                <svg class="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24">
                ...
                <span class="text-xs text-gray-400 font-normal">(perbandingan se-Kabupaten)</span>
            </h2>
            <div class="space-y-3">
                @foreach ($distrikStats as $idx => $dist)
                @php $bc = $barColor($dist['pct']); @endphp
                <div class="{{ $dist['is_mine'] ? 'bg-primary-50 border border-primary-200 -mx-1 px-1 py-1 rounded-xl' : '' }}">
                    <div class="flex items-center justify-between mb-1">
                        <div class="flex items-center gap-2">
                            @if ($dist['is_mine'])
                            <span class="text-xs font-bold text-primary-500 bg-primary-100 px-1.5 py-0.5 rounded">Anda</span>
                            @else
                            <span class="text-xs text-gray-400">#{{ $idx + 1 }}</span>
                            @endif
                            <span class="text-sm font-semibold {{ $dist['is_mine'] ? 'text-primary-700' : 'text-gray-900' }}">
                                {{ $dist['nama'] }}
                            </span>
                            <span class="text-xs text-gray-400">{{ $dist['selesai'] }}/{{ $dist['total'] }} laporan</span>
                        </div>
                        <span class="text-sm font-bold {{ $dist['pct'] >= 90 ? 'text-success' : ($dist['pct'] >= 75 ? 'text-yellow-600' : 'text-error') }}">
                            {{ $dist['pct'] }}%
                        </span>
                    </div>
                    <div class="w-full bg-gray-100 rounded-full h-2">
                        <div class="h-2 rounded-full {{ $bc }} {{ $dist['is_mine'] ? 'opacity-100' : '' }} transition-all"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
            ...
            <div class="space-y-2.5">
                @foreach ($trend as $t)
                @php
                $tWidth  = $total > 0 ? round(($t['total'] / max($trend->max('total'), 1)) * 100) : 0;
                ...
                <div class="flex items-center gap-4">
                    <span class="text-xs text-gray-500 w-24 shrink-0">
                        {{ $t['bulan_short'] }}
                        @if ($isLast) <span class="ml-1 text-[10px] bg-blue-100 text-blue-600 px-1 py-0.5 rounded font-bold">Berjalan</span> @endif
                    </span>
                    ...
                    <div class="w-64 shrink-0 flex items-center gap-3 text-xs text-gray-500">
                        <span>Total <strong class="text-gray-700">{{ $t['total'] }}</strong></span>
                        <span>Selesai <strong class="text-success">{{ $t['selesai'] }}</strong></span>
                        <span>Terlambat <strong class="{{ $t['terlambat'] > 0 ? 'text-error' : 'text-gray-400' }}">{{ $t['terlambat'] }}</strong>
