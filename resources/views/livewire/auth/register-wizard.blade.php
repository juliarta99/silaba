<div
    ...
    class="min-h-[calc(100vh-68px)] flex flex-col items-center justify-center pt-36 pb-12 px-4 sm:px-6 lg:px-8 bg-gray-10"
>

    {{-- ════════════════════════════════════════════
         LOGO + HEADING (sama untuk step 1 & 2)
    ════════════════════════════════════════════ --}}
    @if ($step !== 3)
    ...
    <div class="flex items-center gap-3 mb-8">
        <div class="flex items-center gap-2.5">
            ...
            @else
                <span class="w-8 h-8 rounded-full bg-primary-500 text-white text-sm font-bold flex items-center justify-center shrink-0">1</span>
            @endif
            <span class="text-sm font-medium {{ $step === 1 ? 'text-gray-900 font-semibold' : 'text-gray-500' }} hidden sm:inline">Verifikasi NIK</span>
        </div>
        <div class="w-12 sm:w-16 h-px bg-gray-200"></div>
        <div class="flex items-center gap-2.5">
            <span class="w-8 h-8 rounded-full {{ $step === 2 ? 'bg-primary-500 text-white' : 'bg-gray-100 text-gray-400' }} text-sm font-bold flex items-center justify-center shrink-0">2</span>
            <span class="text-sm font-medium {{ $step === 2 ? 'text-gray-900 font-semibold' : 'text-gray-400' }} hidden sm:inline">Data Lengkap</span>
        </div>
    </div>
    @endif

    {{-- ════════════════════════════════════════════
        STEP 1 — VERIFIKASI NIK (+ Nama manual)
    ════════════════════════════════════════════ --}}
    @if ($step === 1)
    <div class="bg-white p-7 sm:p-8 rounded-2xl shadow-sm border border-gray-100 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-900 mb-1.5">Verifikasi Identitas</h3>
        <p class="text-sm text-gray-500 mb-6 leading-relaxed">
            Masukkan NIK, nama lengkap, dan tanggal lahir sesuai KTP Anda
        </p>

        {{-- Catatan transparansi: ini bukan verifikasi ke database Dukcapil --}}
        {{-- <div class="mb-5 px-4 py-3 rounded-lg bg-blue-50 border border-blue-100">
            <p class="text-xs text-blue-700 leading-relaxed">
                <span class="font-semibold">Catatan:</span>
                Data Anda divalidasi berdasarkan format NIK. Pastikan data yang
                dimasukkan benar dan sesuai KTP, karena akan digunakan untuk
                keperluan pelaporan resmi.
            </p>
        </div> --}}

        <form wire:submit="verifyNik" class="space-y-5">

            {{-- NIK --}}
            <div>
                <label for="nik" class="block text-sm font-medium text-gray-900 mb-2">
                    Nomor Induk Kependudukan (NIK) <span class="text-error">*</span>
                </label>
                <input
                    type="text" id="nik" wire:model="nik"
                    placeholder="Masukkan 16 digit NIK"
                    inputmode="numeric" maxlength="16"
                    autofocus
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-10
                        text-gray-900 placeholder-gray-400 tracking-wide
                        focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500
                        outline-none transition-all duration-150"
                >
                <p class="mt-1.5 text-xs text-gray-400">Satu NIK hanya dapat didaftarkan untuk satu akun aktif</p>
                @error('nik') <p class="mt-1.5 text-xs text-error">{{ $message }}</p> @enderror
            </div>

            {{-- Nama Lengkap — INPUT MANUAL (tidak ada koneksi Disdukcapil) --}}
            <div>
            ...
            <div>
                ...
                <input
                    type="date" id="birth_date" wire:model="birth_date"
                    ...
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-10
                        ...
                        outline-none transition-all duration-150"
                >
                <p class="mt-1.5 text-xs text-gray-400">Harus sesuai dengan kode tanggal lahir pada NIK</p>
                @error('birth_date') <p class="mt-1.5 text-xs text-error">{{ $message }}</p> @enderror
            </div>

            <button
                type="submit"
                wire:loading.attr="disabled"
                wire:target="verifyNik"
            ...
            >
                <span wire:loading.remove wire:target="verifyNik" class="flex items-center gap-2">
                    Lanjutkan
                    <svg class="w-4 h-4" fill="none" viewBox="0 0 16 16" aria-hidden="true">
                        <path d="M3.5 8h9M9 4.5L12.5 8 9 11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                    </svg>
                </span>
                <span wire:loading wire:target="verifyNik" class="flex items-center gap-2">
    ...
    <p class="mt-8 text-center text-sm text-gray-500">
        Sudah punya akun?
        <a href="{{ route('login') }}" wire:navigate class="font-semibold text-primary-500 hover:text-primary-700 hover:underline transition-colors">
            Masuk di sini
        </a>
    </p>
    @endif
    ...
    {{-- ════════════════════════════════════════════
        STEP 2 — DATA LENGKAP (+ Gender radio button)
    ════════════════════════════════════════════ --}}
    @if ($step === 2)
    <div class="bg-white p-7 sm:p-8 rounded-2xl shadow-sm border border-gray-100 w-full max-w-md">
        <h3 class="text-lg font-bold text-gray-900 mb-1.5">Lengkapi Data Anda</h3>
        <p class="text-sm text-gray-500 mb-6 leading-relaxed">
            Lengkapi data kontak dan keamanan akun untuk menyelesaikan pendaftaran.
        </p>

        <form wire:submit="completeRegistration" class="space-y-5" x-data="{ showPw: false, showPwConfirm: false }">

            {{-- Nama (read-only di step ini, sudah diisi di step 1) --}}
            <div>
                ...
                <input
                    ...
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-100
                        text-gray-700 cursor-not-allowed outline-none"
                >
            </div>
            ...
            <div>
                <label class="block text-sm font-medium text-gray-900 mb-2.5">
                    Jenis Kelamin <span class="text-error">*</span>
                </label>

                @if ($genderHint)
                <p class="text-xs text-gray-400 mb-2.5">
                    Berdasarkan kode NIK, kemungkinan jenis kelamin Anda
                    <span class="font-medium text-gray-600">{{ $genderHint === 'male' ? 'Laki-laki' : 'Perempuan' }}</span>.
                    Silakan koreksi jika tidak sesuai.
                </p>
                @endif
                ...
                <div class="grid grid-cols-2 gap-3">
                    ...
                    <label
                        for="gender_male"
                        class="flex items-center gap-2.5 px-4 py-3 rounded-lg border cursor-pointer
                            transition-all duration-150 border-gray-200 bg-gray-10 hover:border-gray-300"
                    >
                        <input
                            type="radio" id="gender_male" name="gender" value="male"
                            wire:model="gender"
                            class="w-4 h-4 text-primary-500 border-gray-300 focus:ring-primary-500 accent-primary-500"
                        >
                    ...
                    >
                        <input
                            type="radio" id="gender_female" name="gender" value="female"
                            wire:model="gender"
                            class="w-4 h-4 text-primary-500 border-gray-300 focus:ring-primary-500 accent-primary-500"
                        >
                        <span class="text-sm font-medium text-gray-700">
                ...
                @error('gender') <p class="mt-1.5 text-xs text-error">{{ $message }}</p> @enderror
            </div>
            ...
            <div>
                ...
                <input
                    ...
                    class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-10
                        ...
                        outline-none transition-all duration-150"
                >
                <p class="mt-1.5 text-xs text-gray-400">Nomor ini akan digunakan untuk verifikasi OTP dan notifikasi laporan</p>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input
                            ...
                            class="w-full px-4 py-3 pr-11 rounded-lg border border-gray-200 bg-gray-10
                                text-gray-900 placeholder-gray-400
                                focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500
                                outline-none transition-all duration-150"
                        >
                        <button type="button" @click="showPw = !showPw"
                            ...
                            <svg class="w-4.5 h-4.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" aria-hidden="true">
            ...
            <label class="flex items-start gap-2.5 cursor-pointer">
                <input
                    ...
                    class="mt-0.5 h-4 w-4 rounded border-gray-300 text-primary-500
                        focus:ring-primary-500 accent-primary-500 shrink-0"
                >
                <span class="text-xs text-gray-600 leading-relaxed">
                    Saya menyetujui
                    <a href="#" class="font-medium text-primary-500 hover:underline">Syarat dan Ketentuan</a>
                    serta
                    <a href="#" class="font-medium text-primary-500 hover:underline">Kebijakan Privasi</a>
                    sesuai UU No. 27 Tahun 2022 tentang Perlindungan Data Pribadi
                </span>
            </label>
            @error('agree') <p class="text-xs text-error">{{ $message }}</p> @enderror

            <div class="flex gap-3">
                <button
                    type="button" wire:click="backToStep1"
                    class="px-5 py-3 rounded-lg border border-gray-200 text-gray-700 text-sm font-semibold
                ...
                <button
                    ...
                    class="flex-1 flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-700
                        ...
                        transition-all duration-150 active:scale-[.98] shadow-sm"
                >
                    <span wire:loading.remove wire:target="completeRegistration" class="flex items-center gap-2">
                        Daftar Sekarang
                        <svg class="w-4 h-4" fill="none" viewBox="0 0 16 16" aria-hidden="true">
                            <path d="M3.5 8h9M9 4.5L12.5 8 9 11.5" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
    ...
    <div class="text-center mb-8">
        <div class="w-20 h-20 rounded-full bg-primary-50 flex items-center justify-center mx-auto mb-5">
            <svg class="w-9 h-9 text-primary-500" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                <path d="M21 11.5a8.38 8.38 0 01-.9 3.8 8.5 8.5 0 01-7.6 4.7 8.38 8.38 0 01-3.8-.9L3 21l1.9-5.7a8.38 8.38 0 01-.9-3.8 8.5 8.5 0 014.7-7.6 8.38 8.38 0 013.8-.9h.5a8.48 8.48 0 018 8v.5z"
                      stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"/>
            </svg>
        </div>
        <h2 class="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">Verifikasi WhatsApp</h2>
        <p class="text-sm text-gray-500 mb-1">Kode OTP telah dikirim ke WhatsApp Anda</p>
        <p class="text-sm font-semibold text-gray-900">{{ $maskedPhone }}</p>

        @if ($whatsappWarning)
            <div class="mt-4 w-full max-w-md px-4 py-3 rounded-lg bg-yellow-50 border border-yellow-200 text-left">
                <p class="text-xs text-yellow-700 leading-relaxed">
                    <span class="font-semibold">Peringatan:</span> {{ $whatsappWarning }}
                </p>
            </div>
        @endif
    </div>
    ...
    <div class="bg-white p-7 sm:p-8 rounded-2xl shadow-sm border border-gray-100 w-full max-w-md">
        ...
        @error('otp')
        <div class="mb-5 px-4 py-3 rounded-lg bg-red-50 border border-red-100 text-sm text-error text-center">
            {{ $message }}
        </div>
        @enderror

        <label class="block text-sm font-medium text-gray-900 text-center mb-4">
            Masukkan Kode OTP (6 Digit)
        </label>

        {{-- 6-digit boxes — pakai Alpine untuk auto-focus, Livewire untuk submit --}}
        <div
            class="flex justify-center gap-2 sm:gap-2.5 mb-5"
            x-data="{
                focusNext(i) {
                    const el = this.$refs[`digit${i+1}`];
                    if (el) el.focus();
                },
                focusPrev(i, e) {
                ...
                handlePaste(e) {
                    ...
                    paste.split('').forEach((d, idx) => {
                        $wire.set(`otpDigits.${idx}`, d, false);
                    });
                    $wire.$refresh();
                    if (paste.length === 6) this.$refs.digit5.focus();
                }
            }"
        ...
        >
            ...
            <input
                type="text"
                x-ref="digit{{ $i }}"
                wire:model.live="otpDigits.{{ $i }}"
                @input="if ($event.target.value) focusNext({{ $i }})"
                @keydown="focusPrev({{ $i }}, $event)"
                maxlength="1"
                inputmode="numeric"
                autocomplete="one-time-code"
                class="w-11 h-12 sm:w-12 sm:h-14 text-center text-lg font-bold
                       rounded-lg border border-gray-200 bg-gray-10 text-gray-900
                       focus:bg-white focus:ring-2 focus:ring-primary-500 focus:border-primary-500
                       outline-none transition-all duration-150"
        ...
        <p class="text-center text-sm text-gray-500 mb-5">
            ...
            <span class="font-semibold text-gray-900">
                {{ sprintf('%d:%02d', intdiv($otpExpiresInSeconds, 60), $otpExpiresInSeconds % 60) }}
            </span>
        </p>
        ...
        <button
            ...
            class="w-full flex items-center justify-center gap-2 bg-primary-500 hover:bg-primary-700
                   ...
                   font-semibold text-sm transition-all duration-150 active:scale-[.98] shadow-sm"
        >
        ...
        <div class="text-center mt-5">
            <p class="text-sm text-gray-500 mb-1">Tidak menerima kode OTP?</p>
            @if ($otpExpiresInSeconds > 0)
                <p class="text-sm font-semibold text-gray-400">
                    Kirim ulang dalam {{ sprintf('%d:%02d', intdiv($otpExpiresInSeconds, 60), $otpExpiresInSeconds % 60) }}
                </p>
            @else
                <button
                    type="button" wire:click="resendOtp"
    ...
    <div class="mt-6 w-full max-w-md px-4 py-3.5 rounded-xl bg-blue-50 border border-blue-100">
    ...
    <button wire:click="backToStep2" class="mt-6 text-sm text-gray-400 hover:text-gray-600 transition-colors">
...
{{--
    JS polling untuk decrement timer setiap 1 detik.
    Pakai wire:poll agar tidak butuh Alpine setInterval manual untuk Livewire call.
--}}
@if ($step === 3 && $otpExpiresInSeconds > 0)
<div wire:poll.1000ms="decrementTimer" class="hidden"></div>
@endif