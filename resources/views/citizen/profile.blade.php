...
$isVerified = (bool) $citizen?->phone_verified_at;

$genderLabel = match($citizen?->gender) {
    'male'   => 'Laki-laki',
    'female' => 'Perempuan',
    default  => '—',
};
...
<div class="bg-gray-10 min-h-[calc(100vh-68px)] py-16">
    <div class="max-w-2xl mx-auto px-4 sm:px-6 pt-8">
        ...
        <div class="flex items-center gap-3 px-4 py-3 rounded-xl bg-green-50 border border-green-100
            ...
            <svg class="w-4 h-4 text-success shrink-0" fill="none" viewBox="0 0 16 16" aria-hidden="true">
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5">
            <div class="flex items-center gap-5">
                ...
                <div class="relative shrink-0" x-data>
                    <div class="w-20 h-20 rounded-full overflow-hidden bg-primary-100
                                flex items-center justify-center">
                        @if ($picture)
                        <img id="avatar-preview" src="{{ $picture }}" alt="{{ $name }}"
                             class="w-full h-full object-cover">
                        @else
                        <span id="avatar-initials"
                              class="text-2xl font-bold text-primary-500 select-none">
                            {{ $initials }}
                        </span>
                        @endif
                    </div>
                    ...
                    {{-- Form upload foto (auto-submit saat file dipilih) --}}
                    <form id="photo-form"
                          action="{{ route('citizen.profile.photo') }}"
                          method="POST"
                ...
                <div>
                    ...
                    <p class="text-sm text-gray-500 mb-2">Warga Terverifikasi</p>
                    @if ($isVerified)
                    <span class="inline-flex items-center gap-1.5 text-xs font-semibold
                                 px-3 py-1.5 rounded-full bg-green-100 text-green-700">
                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 14 14" aria-hidden="true">
                            <path d="M2.5 7l3 3 6-6" stroke="currentColor" stroke-width="1.75"
                                  stroke-linecap="round" stroke-linejoin="round"/>
                        </svg>
                        NIK Terverifikasi
                    </span>
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-5"
            ...
            <div x-show="!editing" class="space-y-5">
                ...
                <div>
                    <label class="flex items-center gap-1.5 text-xs font-semibold text-gray-600 mb-1.5">
                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" viewBox="0 0 16 16" aria-hidden="true">
                            ...
                            <path d="M2 14c0-3.314 2.686-6 6-6s6 2.686 6 6" stroke="currentColor"
                                  stroke-width="1.3" stroke-linecap="round"/>
                        </svg>
                        NIK
                    </label>
                    <div class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-10
                                text-sm text-gray-700 font-mono">
                        {{ $user->identifier }}
                    </div>
                    <p class="text-xs text-gray-400 mt-1.5">NIK tidak dapat diubah setelah verifikasi</p>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1.5">Nama sesuai data Disdukcapil (tidak dapat diubah)</p>
                </div>
                ...
                <div>
                    ...
                    <div class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-10
                                text-sm text-gray-700">
                        {{ $citizen?->birth_date?->translatedFormat('d F Y') ?? '—' }}
                    </div>
                    <p class="text-xs text-gray-400 mt-1.5">Tanggal lahir tidak dapat diubah</p>
                </div>
                    ...
                    <label class="flex items-center gap-1.5 text-xs font-semibold text-gray-600 mb-1.5">
                        <svg class="w-3.5 h-3.5 text-gray-400" fill="none" viewBox="0 0 16 16" aria-hidden="true">
                            <rect x="1.5" y="3.5" width="13" height="9" rx="1.5"
                                  stroke="currentColor" stroke-width="1.3"/>
                            <path d="M1.5 5.5l6.5 4 6.5-4" stroke="currentColor"
                                  stroke-width="1.3" stroke-linecap="round"/>
                        </svg>
                        Email
                    </label>
                    <div class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-white
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-400 mt-1.5">
                        Nomor WhatsApp tidak dapat diubah (terverifikasi dengan OTP)
                    </p>
                </div>
            </div>

            {{-- EDIT MODE (hanya email yang bisa diubah) --}}
            <div x-show="editing" x-cloak>
                <form action="{{ route('citizen.profile.update') }}" method="POST" class="space-y-5">
                    @csrf
                    @method('PATCH')
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $user->identifier }}" readonly
                    ...
                    <div>
                        ...
                        <input type="text"
                               value="{{ $citizen?->birth_date?->translatedFormat('d F Y') ?? '—' }}"
                               readonly
                               class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-100
                                      text-sm text-gray-500 cursor-not-allowed outline-none">
                        <p class="text-xs text-gray-400 mt-1.5">Tanggal lahir tidak dapat diubah</p>
                    ...
                    {{-- Email (EDITABLE) --}}
                    <div>
                        <label for="email" class="flex items-center gap-1.5 text-xs font-semibold text-gray-600 mb-1.5">
                            <svg class="w-3.5 h-3.5 text-gray-400" fill="none" viewBox="0 0 16 16" aria-hidden="true">
                    ...
                    <div>
                        ...
                        <input type="text" value="{{ $citizen?->phone ?? '—' }}" readonly
                               class="w-full px-4 py-3 rounded-lg border border-gray-200 bg-gray-100
                                      text-sm text-gray-500 cursor-not-allowed outline-none">
                        <p class="text-xs text-gray-400 mt-1.5">
                            Nomor WhatsApp tidak dapat diubah (terverifikasi dengan OTP)
                    ...
                    <div class="flex gap-3 pt-2 border-t border-gray-100">
                        <button type="button" @click="editing = false"
                                class="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-700
                                       text-sm font-semibold hover:bg-gray-10 transition-colors">
                            Batal
                        </button>
                        <button type="submit"
                                class="flex-1 py-2.5 rounded-xl bg-primary-500 hover:bg-primary-700
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-6"
            ...
            <div x-show="changingPassword" x-cloak class="mt-5 pt-5 border-t border-gray-100">
                <form action="{{ route('citizen.profile.password') }}" method="POST"
                      class="space-y-4" x-data="{ showCurrent: false, showNew: false, showConfirm: false }">
                    @csrf
                    @method('PATCH')
