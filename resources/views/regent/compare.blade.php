@extends('layouts.app')
@section('title', 'Komparasi Instansi — Kabupaten Badung')
...
<div class="bg-gray-10 min-h-[calc(100vh-68px)] py-16"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 pt-6">
        ...
        <div class="flex items-start justify-between gap-4 mb-6">
            <div>
                ...
                <p class="text-sm text-gray-500 mt-0.5">
                    Perbandingan performa seluruh OPD
                    <span class="font-semibold text-gray-700">Kabupaten Badung</span>
                </p>
            </div>
            <a href="{{ route('regent.reports.compare.export') . '?' . http_build_query(request()->except('_token')) }}"
        ...
        <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 mb-5">
            ...
            <form method="GET" action="{{ route('regent.reports.compare') }}">
                ...
                <div x-show="showCustomDate" x-cloak>
                    ...
                    <button type="submit"
                            class="px-5 py-2.5 rounded-xl bg-primary-500 hover:bg-primary-700 text-white text-sm font-semibold transition-colors">
                        Terapkan Filter
                    </button>
                </div>
                ...
                <div class="mt-3 flex items-center justify-between">
                    <p class="text-xs text-gray-500 flex items-center gap-1.5">
                        ...
                        @endif
                        @if ($dateFrom && $dateTo)
                            {{ \Carbon\Carbon::parse($dateFrom)->translatedFormat('j M Y') }} —
                            {{ \Carbon\Carbon::parse($dateTo)->translatedFormat('j M Y') }}
                        @endif
                    </p>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100 bg-gray-10">
                            ...
                            <th class="text-center px-3 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide min-w-[110px]">Completion</th>
                            <th class="text-center px-3 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide">Rata-rata</th>
                            ...
                            <th class="text-left px-3 py-3.5 text-xs font-semibold text-gray-500 uppercase tracking-wide min-w-[160px]">Kecamatan</th>
                    ...
                    <tbody class="divide-y divide-gray-50">
                        ...
                        $rank      = $idx + 1;
                        $isTop3    = $rank <= 3;
                        $rateClr   = $d->completionRate >= 90 ? 'text-success' : ($d->completionRate >= 85 ? 'text-yellow-600' : 'text-error');
                        $barClr    = $d->completionRate >= 90 ? 'bg-success'   : ($d->completionRate >= 85 ? 'bg-yellow-400'   : 'bg-error');
                        $trendStr  = $d->trend !== null ? ($d->trend > 0 ? '+' . $d->trend : (string)$d->trend) . '%' : '—';
                        ...
                        <tr class="hover:bg-gray-10 transition-colors {{ $d->completionRate < 85 ? 'bg-red-50/40' : '' }}">
                            ...
                            <td class="px-3 py-4 text-center font-semibold text-gray-700">{{ number_format($d->total) }}</td>

                            <td class="px-3 py-4 text-center">
                                <p class="font-bold text-success">{{ number_format($d->selesai) }}</p>
                                <p class="text-[10px] text-gray-400">{{ $d->proses }} proses, {{ $d->terlambat }} terlambat</p>
                            </td>

                            <td class="px-3 py-4">
                                <p class="text-center font-bold text-base {{ $rateClr }} mb-1">{{ $d->completionRate }}%</p>
                                <div class="w-full bg-gray-100 rounded-full h-1.5">
                                    <div class="h-1.5 rounded-full {{ $barClr }}" style="width:{{ $d->completionRate }}%"></div>
                                </div>
                            </td>
                            ...
                            <td class="px-3 py-4 text-center text-xs">
                                <span class="font-semibold text-gray-700">{{ $d->petugasAktif }}</span>
                                <span class="text-gray-400">/{{ $d->petugasTotal }}</span>
                                <p class="text-[10px] text-gray-400">Aktif/Total</p>
                            </td>

                            {{-- Kolom Kecamatan — tambahan untuk Bupati --}}
                            <td class="px-3 py-4">
                                @if (count($d->kecamatanNames) > 0)
                                <div class="flex flex-wrap gap-1">
                                    @foreach (array_slice($d->kecamatanNames, 0, 3) as $kec)
                                    <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-blue-50 text-blue-600">
                                        {{ $kec }}
                                    </span>
                                    @endforeach
                                    @if (count($d->kecamatanNames) > 3)
                                    <span class="text-[10px] font-medium px-1.5 py-0.5 rounded-full bg-gray-100 text-gray-500">
                                        +{{ count($d->kecamatanNames) - 3 }} lagi
                                    </span>
                                    @endif
                                </div>
                                <p class="text-[10px] text-gray-400 mt-0.5">{{ count($d->kecamatanNames) }} kecamatan</p>
                                @else
                            ...
                            <td class="px-3 py-4 text-center">
                                <span class="text-sm font-bold {{ $trendClr }}">{{ $trendIcon }} {{ $trendStr }}</span>
                            </td>
                        </tr>
                        @endforeach
                        ...
                        @if ($data->isEmpty())
                        <tr>
                            <td colspan="10" class="text-center py-12 text-sm text-gray-400">Tidak ada data OPD</td>
                        </tr>
                        @endif
                    </tbody>
                </table>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                @php $totalDept = array_sum($distribusi); @endphp
                <div class="space-y-4">
                    @foreach ([
                    ...
                    <div class="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-green-50 border border-green-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-green-700 leading-relaxed">
                                ...
                                memimpin dengan completion rate <strong>{{ $topPerformer->completionRate }}%</strong>
                    ...
                    <div class="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-red-50 border border-red-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-red-700 leading-relaxed">
                                ...
                                Diperlukan evaluasi dan perbaikan sistem segera.
                            </p>
                        </div>
                    </div>
                    @endif
                    ...
                    <div class="flex items-start gap-3 px-4 py-3.5 rounded-xl bg-blue-50 border border-blue-100">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-blue-700 leading-relaxed">
                                Selisih antara OPD terbaik dan terburuk adalah <strong>{{ $gap }}%</strong>.
