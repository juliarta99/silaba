{{--
    SILABU Navbar — Public + Non-Admin roles only
    Admin / Super Admin pakai layout sidebar terpisah (tidak pakai component ini)

    Persona:
    1. guest           → Beranda | Laporan Terbaru | Tentang | Masuk | [Laporkan Sekarang]
    2. citizen         → Beranda | Laporan Terbaru | Tentang | [Laporkan Sekarang] | avatar▾
                         dropdown: Dashboard Saya · Laporan Saya · Klaim Reward · Logout
    3. field_officer   → Dashboard | [Tugas Saya] | avatar▾
                         dropdown: Profile · Logout
    4. supervisor      → Dashboard | Daftar Laporan | Penugasan Petugas | Kelola Instansi | [Performa] | Peta Sebaran | avatar▾
                         dropdown: Profile · Logout
    5. head_of_dept    → Dashboard | Daftar Laporan | Kelola Instansi | [Performa] | Peta Sebaran | avatar▾
                         dropdown: Profile · Logout
    6. regent          → Dashboard | Daftar Laporan | Komparasi Instansi | Peta Sebaran | [Rekomendasi Prioritas] | avatar▾
                         dropdown: Profile · Logout

    Mobile: avatar selalu tampil di kiri hamburger (tidak hilang)
    Dropdown avatar: melayang, tidak mendorong konten
--}}
...
@php
    ...
    // Profile route per persona
    $profileRoute = match(true) {
        $isCitizen      => null,   // citizen tidak punya halaman profile khusus (pakai dropdown saja)
    ...
    // Label role untuk header dropdown
...
>
    <div class="max-w-7xl mx-auto px-5 sm:px-6 h-17 flex items-center justify-between gap-3">
        ...
        <a href="{{ route('home') }}" aria-label="SILABU" class="shrink-0">
            <svg class="h-6.5 w-auto" viewBox="0 0 159 40" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="hidden lg:flex items-center gap-0.5">
            ...
            {{-- ─── 3. PETUGAS LAPANGAN (field_officer) ─── --}}
            @if ($isFieldOfficer)
                <x-navbar-link :href="route('employee.field-officer.dashboard')"
                               :active="request()->routeIs('employee.field-officer.dashboard')">
                    Dashboard
                </x-navbar-link>
                <x-navbar-primary-btn :href="route('employee.field-officer.assignments.index')" class="ml-1">
                    Tugas Saya
                </x-navbar-primary-btn>
            @endif

            {{-- ─── 4. SUPERVISOR ─── --}}
            @if ($isSupervisor)
                <x-navbar-link :href="route('employee.supervisor.dashboard')"
                               :active="request()->routeIs('employee.supervisor.dashboard')">Dashboard</x-navbar-link>
                <x-navbar-link :href="route('employee.shared.reports.index')"
                               :active="request()->routeIs('employee.shared.reports.*') && !request()->routeIs('employee.shared.reports.map')">Daftar Laporan</x-navbar-link>
                <x-navbar-link :href="route('employee.shared.assignments.index')"
                               :active="request()->routeIs('employee.shared.assignments.*')">Penugasan Petugas</x-navbar-link>
                <x-navbar-link :href="route('employee.shared.departments.index')"
                               :active="request()->routeIs('employee.shared.departments.*')">Kelola Instansi</x-navbar-link>
                <x-navbar-primary-btn :href="route('employee.shared.reviews.index')" class="ml-1">Performa</x-navbar-primary-btn>
                <x-navbar-link :href="route('employee.shared.reports.map')"
                               :active="request()->routeIs('employee.shared.reports.map')">Peta Sebaran</x-navbar-link>
            @endif

            {{-- ─── 5. KEPALA DINAS (head_of_department) ─── --}}
            @if ($isHoD)
                <x-navbar-link :href="route('employee.head.dashboard')"
                               :active="request()->routeIs('employee.head.dashboard')">Dashboard</x-navbar-link>
                <x-navbar-link :href="route('employee.shared.reports.index')"
                               :active="request()->routeIs('employee.shared.reports.*') && !request()->routeIs('employee.shared.reports.map')">Daftar Laporan</x-navbar-link>
                <x-navbar-link :href="route('employee.shared.departments.index')"
                               :active="request()->routeIs('employee.shared.departments.*')">Kelola Instansi</x-navbar-link>
            ...
            {{-- ─── 6 & 7. CAMAT / BUPATI (regent) ─── --}}
            @if ($isRegent)
                <x-navbar-link :href="route('regent.dashboard')"
                               :active="request()->routeIs('regent.dashboard')">Dashboard</x-navbar-link>
                <x-navbar-link :href="route('regent.reports.index')"
                               :active="request()->routeIs('regent.reports.index')">Daftar Laporan</x-navbar-link>
                <x-navbar-link :href="route('regent.departments.compare')"
                               :active="request()->routeIs('regent.departments.compare')">Komparasi Instansi</x-navbar-link>
                <x-navbar-link :href="route('regent.reports.map')"
                               :active="request()->routeIs('regent.reports.map')">Peta Sebaran</x-navbar-link>
                <x-navbar-primary-btn :href="route('regent.reports.priority')" class="ml-1">
                    Rekomendasi Prioritas
                </x-navbar-primary-btn>
            @endif

            {{-- ─── AVATAR + DROPDOWN DESKTOP (semua yang login) ─── --}}
            @if ($isLoggedIn)
            <div class="relative ml-3 shrink-0" x-data="{ open: false }">
                ...
                <button
                    ...
                    class="flex items-center gap-1.5 p-0.5 rounded-full
                           focus:outline-none focus-visible:ring-2 focus-visible:ring-primary-500 focus-visible:ring-offset-2 transition-all"
                    :aria-expanded="open"
                    aria-haspopup="true"
                ...
                >
                    ...
                    @else
                        <span class="w-9 h-9 rounded-full bg-primary-500 text-white text-xs font-bold
                                     flex items-center justify-center tracking-wide select-none shrink-0">
                            {{ $initials }}
                        </span>
                    @endif
                    {{-- Chevron --}}
                ...
                <div
                    ...
                    class="absolute right-0 top-[calc(100%+10px)] w-56
                           bg-white border border-gray-100 rounded-xl shadow-lg
                           py-1.5 z-60 origin-top-right"
                    role="menu"
                >
                    {{-- Header info user --}}
                    <div class="px-3.5 py-2.5">
                        <div class="flex items-center gap-2.5 mb-0.5">
                            @if ($picture)
                                <img src="{{ $picture }}" alt=""
                                     class="w-8 h-8 rounded-full object-cover border border-primary-100 shrink-0">
                            @else
                    ...
                    @else
                        @if ($profileRoute)
                            <x-navbar-dropdown-item :href="$profileRoute">Profile</x-navbar-dropdown-item>
                        @endif
                    @endif
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        ...
                        <button type="submit" role="menuitem"
                            class="w-full text-left px-3.5 py-2 text-sm font-medium text-error
                                   rounded-lg hover:bg-red-50 transition-colors duration-100">
                            Logout
                        </button>
                    </form>
                </div>
        ...
        <div class="lg:hidden flex items-center gap-2 shrink-0">
            ...
            {{-- Avatar mobile — selalu tampil jika login --}}
            @if ($isLoggedIn)
            <div class="relative" x-data="{ open: false }">
                ...
                {{-- Dropdown melayang — tidak mendorong konten --}}
                <div
                    x-show="open"
                    x-transition:enter="transition ease-out duration-100"
                    x-transition:enter-start="opacity-0 scale-95 -translate-y-1"
                    ...
                    class="absolute right-0 top-[calc(100%+8px)] w-56
                           bg-white border border-gray-100 rounded-xl shadow-lg
                           py-1.5 z-60 origin-top-right"
                    role="menu"
                >
            ...
            <button
                ...
                class="flex flex-col justify-center gap-1.25 w-9 h-9 p-1.5 rounded-lg
                       hover:bg-primary-50 transition-colors"
                :aria-expanded="mobileOpen"
                aria-label="Toggle navigasi"
            >
                <span class="block w-full h-0.5 bg-primary-500 rounded transition-all duration-200"
                      :class="{ 'translate-y-1.75 rotate-45': mobileOpen }"></span>
                <span class="block w-full h-0.5 bg-primary-500 rounded transition-all duration-200"
    ...
    <div
        ...
        class="lg:hidden fixed top-17 inset-x-0 z-40
               bg-white border-b border-gray-100 shadow-md
               px-4 pb-5 pt-3 flex flex-col gap-0.5
